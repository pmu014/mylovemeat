...
class ProductsController {
  ...
  cartProduct = async(req,res) =>{
    ...
    const productid = await this.productsServices.productIdFind(userId,productId);
   
    if(productid !== null){
      return res.status(409).json({errorMessage : "이미 장바구니에 상품이 존재합니다"})
  } 
    ...
    if (checkErrorMessage(carts)) {
      return res
      .status(carts.code)
      .json({ errorMessage: carts.message });
    }
    res.status(201).json({message : "장바구니에 담겼습니다."});
    }catch(error){
      console.log(error)
        return res.status(400).send({ errorMessage: "입력한 요청이 잘못되었습니다" });
    }
  }
}
